...
def ba_chuso(abc):
    ...
    if so_chuc == 0 and (abc%100)%10 != 0:
        doc_ba.append(print(hang_tram[abc//100] + "linh " + mot_chuso[(abc%100)%10]))
    else:
        doc_ba.append(print(hang_tram[abc//100]))
...
def so_lon(a):
    ...
    for i in range(0, (z-1)):
        ...
        cach_doc.append(print(hang[z - i - 1]))
    i= z-1
    ai = a[(l - 3 * (z - i)):(l - 3 * (z - i) + 3)]
    so_ai = int("".join([str(j) for j in ai]))
    if so_ai <= 9:
        cach_doc.append(print("le"))
        cach_doc.append(print(mot_chuso[so_ai]))
    else :
